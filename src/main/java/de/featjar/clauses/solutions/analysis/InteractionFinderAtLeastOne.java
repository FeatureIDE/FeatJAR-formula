...
public class InteractionFinderAtLeastOne extends AbstractInteractionFinder {
	...
	public InteractionFinderAtLeastOne(Collection<LiteralList> sample, SolutionUpdater configurationGenerator,
	...
	private LiteralList findConfig(List<LiteralList> interactionsAll) {
		if (interactionsAll.size() < 5000) {
			...
			LiteralList bestConfig = findBestConfig(interactionsAll,
					getRandomConfigs((int) Math.ceil(2 * Math.log(interactionsAll.size()))));

			return bestConfig != null ? bestConfig
	...
	private List<LiteralList> getAtLeastOneConfigs(List<LiteralList> interactionsAll) {
		List<LiteralList> potentialConfs = new ArrayList<>();
		interationLoop: for (int i = 0; i < interactionsAll.size(); i++) {
			LiteralList interaction = interactionsAll.get(i);
			if (potentialConfs.parallelStream().filter(c -> c.containsAll(interaction)).findAny().isPresent()) {
	...
	private LiteralList findBestConfig(List<LiteralList> interactionsAll, List<LiteralList> potentialConfs) {
		...
		for (LiteralList config : potentialConfs) {
			final double ratio = Math
					.abs(0.5 - (((double) interactionsAll.parallelStream().filter(config::containsAll).count())
