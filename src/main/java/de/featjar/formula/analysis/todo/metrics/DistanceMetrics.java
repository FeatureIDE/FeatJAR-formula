 ...
 */
package de.featjar.formula.analysis.todo.metrics;
...
public class DistanceMetrics extends AggregatableMetrics {
    ...
    protected double[] computeValues() {
        final BooleanSolutionList solutions = sample.getSolutionList();
        final int size = solutions.size();
        final double[] values = new double[(size * (size - 1)) >> 1];
        int index = 0;
        for (int i = 0; i < (size - 1); i++) {
            final int[] literals1 = solutions.get(i).getIntegers();
            for (int j = i + 1; j < size; j++) {
                values[index++] =
                        function.computeDistance(literals1, solutions.get(j).getIntegers());
            }
        }
        return values;
    }
