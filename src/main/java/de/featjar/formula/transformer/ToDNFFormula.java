 ...
 */
public class ToDNFFormula implements Computation<Formula> {
    protected final Computation<Formula> nnfFormulaComputation;
    protected int maximumNumberOfLiterals;

    public ToDNFFormula(Computation<Formula> nnfFormulaComputation) {
    ...
    public FutureResult<Formula> compute() {
        return nnfFormulaComputation.get().thenComputeResult(((formula, monitor) -> {
            ...
            if (normalFormTester.isNormalForm()) {
                ...
                ToNormalFormFormula formulaToDistributiveNFFormula = new ToNormalFormFormula(Computation.of((formula instanceof Or) ? formula : new Or(formula), monitor), Formula.NormalForm.DNF);
