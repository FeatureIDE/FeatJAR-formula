...
import org.spldev.formulas.parse.ErrorType.*;
import org.spldev.formulas.parse.Symbols.*;
...
import org.spldev.utils.io.formats.ParseProblem.*;
...
public class NodeReader {
	...
	private List<ParseProblem> problemList;
	...
	public void setVariableNmes(Collection<String> variableNames) {
	 ...
	 * @param problemList   A list for providing detailed information on parsing
	 *                      problems
	 * @return The parsed formula
	 */
	public Optional<Formula> read(String formulaString, List<ParseProblem> problemList) {
		this.problemList = problemList;
		if (formulaString == null) {
			problemList.add(new ParseProblem(new ParseException(ErrorMessage.NULL_CONSTRAINT.getMessage(), 0), 0));
			return Optional.empty();
		}
		try {
			return Optional.ofNullable(parseNode(formulaString));
		} catch (final ParseException e) {
			problemList.add(new ParseProblem(e, 0));
			switch (ignoreUnparsableSubExpressions) {
			case KEEP:
				return Optional.of(new ErrorLiteral(formulaString));
			case REMOVE:
			case THROW:
				return Optional.empty();
			default:
	 ...
	 * @return The parsed formula
	 */
	public Optional<Formula> read(String formulaString) {
		return read(formulaString, new ArrayList<>());
	}
