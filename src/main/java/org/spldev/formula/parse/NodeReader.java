...
import org.spldev.formula.parse.ErrorType.*;
import org.spldev.formula.parse.Symbols.*;
...
public class NodeReader {
	 ...
	 */
	public ParseResult<Formula> read(String formulaString) {
		problemList = new ArrayList<>();
		if (formulaString == null) {
			return ParseResult.empty(new ParseProblem(new ParseException(ErrorMessage.NULL_CONSTRAINT.getMessage(), 0),
				0));
		}
		try {
			return ParseResult.of(parseNode(formulaString));
		} catch (final ParseException e) {
			problemList.add(new ParseProblem(e, 0));
			switch (ignoreUnparsableSubExpressions) {
			case KEEP:
				return ParseResult.of(new ErrorLiteral(formulaString));
			case REMOVE:
			case THROW:
				return ParseResult.empty(problemList);
			default:
